...
let main argv =
    Console.WriteLine("Chip 8 Debugger Version 1.0 - s_society")
    let onExit obj evt =
        Environment.Exit(1)
    let onLoad obj evt =
        Console.WriteLine("onLoad pressed")
        ...
        Console.WriteLine("Going soon to the main loop")
        ...
        Console.WriteLine("Turned black")
    ...
    let Open = new ToolStripMenuItem("Load",null,new EventHandler(onLoad))
    let Exit = new ToolStripMenuItem("Exit",null, new EventHandler(onExit))
    chip8.File.DropDownItems.Add(Open)
    chip8.File.DropDownItems.Add(Exit)
    chip8.Menu.Items.Add(chip8.File);
    chip8.form.BackColor <- Color.Black
    chip8.form.Controls.Add(chip8.Menu)
